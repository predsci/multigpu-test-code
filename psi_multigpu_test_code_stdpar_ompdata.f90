!#######################################################################
!
! ****** PSI_MULTIGPU_TEST_CODE_STDPAR_OMPDATA
!
!     This code mimics the basic MPI+DC tasks of PSI's
!     MAS Solar MHD code with OpenMP Target data movement.
!
...
!
!     The code uses an MPI shared communicator to set the GPU device
!     number using the 'set device' ACC pragma and OpenMP Target equivalent.
!
!     This code assumes you launch it with the number of MPI ranks per
!     node = number of GPUs per node
!     (e.g. for a dual socket node with 4 GPUs: mpiexec -npersocket 2)
!
...
!
! ****** Set GPU device number for current rank.
!
...
!$omp target enter data map(alloc:sbuf1r,sbuf2r,sbuf1t,sbuf2t,sbuf1p,sbuf2p, &
!$omp                             rbuf1r,rbuf2r,rbuf1t,rbuf2t,rbuf1p,rbuf2p)
!
...
!
!$omp target exit data map(delete:sbuf1r,sbuf2r,sbuf1t,sbuf2t,sbuf1p,sbuf2p, &
!$omp                             rbuf1r,rbuf2r,rbuf1t,rbuf2t,rbuf1p,rbuf2p)
...
!$omp target enter data map(alloc:v,v%r,v%t,v%p)
!
...
!
      if (iamp0) then
        print*,"Run completed!"
!$omp target update from(v%r,v%t,v%p)
...
!
!$omp target exit data map(delete:v%r,v%t,v%p,v)
